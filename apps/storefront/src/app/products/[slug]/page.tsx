'use client';

import { useState, useEffect } from 'react';
import { useParams, useRouter } from 'next/navigation';
import Link from 'next/link';
import { Product } from '@/lib/api';

const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001/api/v1';

export default function ProductDetailPage() {
  const params = useParams();
  const router = useRouter();
  const slug = params.slug as string;

  const [product, setProduct] = useState<Product | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [selectedVariant, setSelectedVariant] = useState<string | null>(null);
  const [quantity, setQuantity] = useState(1);

  useEffect(() => {
    async function loadProduct() {
      try {
        setLoading(true);
        const res = await fetch(`${API_URL}/storefront/products/${slug}`);
        
        if (!res.ok) {
          if (res.status === 404) {
            setError('Product not found');
          } else {
            setError('Failed to load product');
          }
          return;
        }

        const data = await res.json();
        
        if (data.success && data.data) {
          setProduct(data.data);
          if (data.data.variants && data.data.variants.length > 0) {
            setSelectedVariant(data.data.variants[0].id);
          }
        } else {
          setError('Product not found');
        }
      } catch (err) {
        setError('Failed to load product');
        console.error(err);
      } finally {
        setLoading(false);
      }
    }

    if (slug) {
      loadProduct();
    }
  }, [slug]);

  if (loading) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          <div className="inline-flex items-center gap-2 text-gray-600">
            <div className="w-4 h-4 bg-amber-500 rounded-full animate-pulse" />
            <span>Loading product...</span>
          </div>
        </div>
      </div>
    );
  }

  if (error || !product) {
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center px-4">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">404</h1>
          <p className="text-gray-600 mb-8">{error || 'Product not found'}</p>
          <Link
            href="/products"
            className="inline-block px-6 py-3 bg-amber-500 text-white rounded-lg hover:bg-amber-600 transition-colors"
          >
            Back to Products
          </Link>
        </div>
      </div>
    );
  }

  const displayPrice = parseFloat(String(product.salePrice || product.regularPrice));
  const regularPrice = parseFloat(String(product.regularPrice));
  const hasDiscount = product.salePrice && parseFloat(String(product.salePrice)) < regularPrice;
  const mainImage = product.media?.find((m) => m.isMain) || product.media?.[0];

  const handleAddToCart = () => {
    // TODO: Implement add to cart
    alert('Cart functionality coming soon!');
  };

  return (
    <main className="min-h-screen">
      {/* Breadcrumb */}
      <div className="bg-luxury-beige border-b border-luxury-sand py-6 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <nav className="flex items-center gap-2 text-sm text-luxury-brown">
            <Link href="/" className="hover:text-luxury-gold transition-colors">
              Home
            </Link>
            <span className="text-luxury-sand">/</span>
            <Link href="/products" className="hover:text-luxury-gold transition-colors">
              Products
            </Link>
            <span className="text-luxury-sand">/</span>
            <span className="text-luxury-charcoal font-medium">{product.name}</span>
          </nav>
        </div>
      </div>

      {/* Product Detail */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
          {/* Images */}
          <div>
            <div className="relative overflow-hidden bg-white aspect-square mb-4 border border-luxury-sand shadow-sm">
              {mainImage?.url ? (
                <img
                  src={mainImage.url}
                  alt={mainImage.altText || product.name}
                  className="w-full h-full object-cover"
                />
              ) : (
                <div className="w-full h-full bg-gradient-to-br from-luxury-beige to-luxury-sand flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-6xl mb-4 text-luxury-gold">🖼️</div>
                    <span className="text-luxury-brown text-lg font-serif">No image available</span>
                  </div>
                </div>
              )}
              {hasDiscount && (
                <div className="absolute top-6 right-6 bg-luxury-gold text-white px-4 py-2 font-serif text-sm tracking-wider">
                  SALE
                </div>
              )}
            </div>
          </div>

          {/* Details */}
          <div>
            <h1 className="text-5xl font-serif font-light text-luxury-charcoal mb-6 leading-tight">
              {product.name}
            </h1>

            <div className="flex items-baseline gap-4 mb-8 pb-8 border-b border-luxury-sand">
              <span className="text-4xl font-serif text-luxury-charcoal">
                ${displayPrice.toFixed(2)}
              </span>
              {hasDiscount && (
                <span className="text-2xl text-luxury-brown/50 line-through font-light">
                  ${regularPrice.toFixed(2)}
                </span>
              )}
            </div>

            <div className="mb-8">
              <p className="text-luxury-brown leading-relaxed text-lg">{product.description}</p>
            </div>

            {/* SKU */}
            <div className="mb-8 text-sm text-luxury-brown/70 tracking-wide">
              SKU: {product.sku}
            </div>

            {/* Dimensions */}
            {(product.lengthCm || product.widthCm || product.heightCm) && (
              <div className="mb-8 p-6 bg-white border border-luxury-sand">
                <h3 className="font-serif text-lg text-luxury-charcoal mb-4">Dimensions</h3>
                <div className="grid grid-cols-3 gap-6 text-sm">
                  {product.lengthCm && (
                    <div>
                      <p className="text-luxury-brown/60 mb-1 uppercase text-xs tracking-wider">Length</p>
                      <p className="font-medium text-luxury-charcoal">{product.lengthCm} cm</p>
                    </div>
                  )}
                  {product.widthCm && (
                    <div>
                      <p className="text-luxury-brown/60 mb-1 uppercase text-xs tracking-wider">Width</p>
                      <p className="font-medium text-luxury-charcoal">{product.widthCm} cm</p>
                    </div>
                  )}
                  {product.heightCm && (
                    <div>
                      <p className="text-luxury-brown/60 mb-1 uppercase text-xs tracking-wider">Height</p>
                      <p className="font-medium text-luxury-charcoal">{product.heightCm} cm</p>
                    </div>
                  )}
                </div>
              </div>
            )}

            {/* Variants */}
            {product.variants && product.variants.length > 0 && (
              <div className="mb-8">
                <h3 className="font-serif text-lg text-luxury-charcoal mb-4">Select Variant</h3>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  {product.variants.map((variant) => (
                    <button
                      key={variant.id}
                      onClick={() => setSelectedVariant(variant.id)}
                      className={`px-5 py-3 border transition-all text-sm tracking-wide ${
                        selectedVariant === variant.id
                          ? 'border-luxury-gold bg-luxury-gold text-white'
                          : 'border-luxury-sand bg-white hover:border-luxury-gold text-luxury-brown'
                      }`}
                    >
                      {variant.name}
                    </button>
                  ))}
                </div>
              </div>
            )}

            {/* Quantity */}
            <div className="mb-10">
              <h3 className="font-serif text-lg text-luxury-charcoal mb-4">Quantity</h3>
              <div className="flex items-center gap-4">
                <button
                  onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  className="w-12 h-12 border border-luxury-sand bg-white hover:border-luxury-gold transition-colors text-luxury-brown"
                >
                  −
                </button>
                <span className="text-xl font-serif w-16 text-center text-luxury-charcoal">{quantity}</span>
                <button
                  onClick={() => setQuantity(quantity + 1)}
                  className="w-12 h-12 border border-luxury-sand bg-white hover:border-luxury-gold transition-colors text-luxury-brown"
                >
                  +
                </button>
              </div>
            </div>

            {/* Add to Cart */}
            <div className="flex gap-4 mb-10">
              <button
                onClick={handleAddToCart}
                className="flex-1 px-10 py-5 bg-luxury-gold text-white font-serif text-lg tracking-wider uppercase hover:bg-luxury-darkGold transition-all hover:shadow-lg"
              >
                Add to Cart
              </button>
              <button className="px-6 py-5 border border-luxury-sand bg-white hover:border-luxury-gold transition-colors">
                <span className="text-2xl text-luxury-gold">♡</span>
              </button>
            </div>

            {/* Features */}
            <div className="border-t border-luxury-sand pt-8">
              <div className="grid grid-cols-1 gap-5 text-sm">
                <div className="flex items-center gap-4">
                  <span className="text-2xl text-luxury-gold">✓</span>
                  <span className="text-luxury-brown">Authentic craftsmanship</span>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-2xl text-luxury-gold">✓</span>
                  <span className="text-luxury-brown">Worldwide shipping available</span>
                </div>
                <div className="flex items-center gap-4">
                  <span className="text-2xl text-luxury-gold">✓</span>
                  <span className="text-luxury-brown">Premium quality guarantee</span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </main>
  );
}
